...
def get_file_path(outdir: str, dataset: str, file_index: int = 0) -> str:
    """Return the optical output file path for a given dataset and file index."""
    return os.path.join(outdir, 'sensor_optical',
                        f'{dataset}_sensor_optical_{file_index:04d}.h5')


def build_viz_config(optical_path: str) -> Dict:
    """Build a minimal config dict from HDF5 file metadata."""
    with h5py.File(optical_path, 'r') as f:
...
def load_event(
    ...
    """Load one event from a production optical file.

    Parameters
    ----------
    optical_path : str
        Path to ``{dataset}_sensor_optical_{NNNN}.h5``.
    event_idx : int
...
def list_events(optical_path: str) -> List[str]:
    """Return sorted event keys in an optical file."""
    with h5py.File(optical_path, 'r') as f:
